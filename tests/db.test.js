...
test('Postgres schema, bootstrap, upserts, deletion cascade and read-back through API handlers',async()=>{
 const mem=newDb();mem.public.registerFunction({name:'clock_timestamp',returns:DataType.timestamptz,implementation:()=>new Date(),impure:true});const {Pool}=mem.adapters.createPg();const pool=new Pool();const db=createStore(pool);const handlers=createHandlers({db,now:()=>L.weekFirstKickoff(1)-10000,adminKey:()=> 'test-admin'});
