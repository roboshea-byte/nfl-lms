...
test('home-screen install metadata and icon files are complete',()=>{
 ...
 const share=fs.readFileSync('assets/share/nfl-lms-whatsapp-get-started.png');assert.equal(share.toString('ascii',1,4),'PNG');assert.equal(share.readUInt32BE(16),1080);assert.equal(share.readUInt32BE(20),1920);assert.match(fs.readFileSync('scripts/build.js','utf8'),/nfl-lms-whatsapp-get-started\.png/);
});
test('mobile navigation, refresh, help, privacy and announcements stay wired',()=>{
 ...
 assert.match(current,/<a class="brand" id="homeBrand" href="\/"/);assert.match(current,/\.refresh-app\{position:absolute/);assert.doesNotMatch(current,/\.refresh-app\{position:fixed/);
  ...
  assert.match(current,/\['reminders','Reminders'\]/);assert.match(current,/function renderAdminReminders\(/);assert.match(current,/function reminderDetails\(/);assert.match(current,/Missing Week \$\{week\} picks/);assert.match(current,/Awaiting payment/);assert.match(current,/Not entered this round/);assert.match(current,/function shareReminderMessage\(/);assert.match(current,/https:\/\/wa\.me\/\?text=/);assert.match(current,/Share pick reminder/);assert.match(current,/How do I send WhatsApp reminders\?/);
