...
- Every settled NFL week records when the last result was finalised and waits exactly six hours before the following week’s picks unlock. If the settled week decides the competition, the same timer controls the next round. An open app checks on its 45-second live poll and a closed app checks on the next visit or refresh.
...
- All 31 automated tests pass. Isolated Chromium flows verified the Week 1 review screen, the exact Week 2 unlock, completed-round review, the early admin transition, an empty Round 2, member deep-linking into Round 1, all 16 archived results, and no page overflow at 320, 390, 768 and 1,440px. No production data was used.
...
- Admin Settings can publish or hide a validated 1,000-character member announcement in four styles: yellow Competition update, green Payment reminder, light-blue General update and red Important update. Public API state includes the announcement while retaining existing pick and email privacy.
- Role promotion now uses an explicit Save button and keeps the member’s active session valid. Promoted admins receive Admin home, Entries & payments, Members, Manage picks, Results, Overview and the role-aware Admin guide. Settings, role assignment, imports, backups, restore and reset remain owner-only in both the interface and API. Owner takeover preserves an admin target’s real role and opens the same admin screen.
- `/privacy` is available through local and Vercel routing and explains names, email addresses, password hashing, competition records, visibility and correction/removal requests.
- Fixture cards reserve and contain a right-aligned date/time column. Chromium checks at 390px found all 16 dates inside their cards with no document overflow; the desktop check at 1,440px also found no overflow. Dashboard order, Standings content, announcement rendering, admin controls and the four privacy sections were checked in the isolated local app.
- All 31 automated tests pass, including announcement validation and persistence, privacy routing and Dashboard composition.
...
Chromium browser flows passed locally and against a protected Vercel preview with a separate Neon schema: owner setup, immediate member signup, unpaid dashboard, admin payment activation, pick/change persistence and owner role management. Layout checks passed at 320, 393, 430, 768 and 1440 CSS pixels. WebKit iPhone signup fits with readable form controls. Preview fixture accounts were removed after verification.
...
- Entries & payments now constrains Label to a compact number-sized column and field. Unpaid rows show a separate Mark paid button; paid rows retain a Paid control for correcting the payment status. Paid active rows in Manage picks now read Paid instead of Eligible.
