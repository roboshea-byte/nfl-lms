...
The live app now uses owner, admin and member accounts. See [account setup and operations](docs/ACCOUNTS.md). Signup requires first name, last name, email and password. Members can add several independently managed entries from Account; the app numbers them under the member’s full name and every new entry must be marked paid by an admin before it can pick. Dashboard is the home page and includes the rules, 32-team grid and current weekly fixtures. Member navigation is Dashboard, Picks, Results and Standings; Picks also contains pick history. Standings lists each active entry on a separate line with every revealed team it has picked, alongside the prize pot, weekly status and live feed. On phones these four sections use a fixed icon tab bar at the bottom. The site can be saved to a phone home screen as NFL LMS with its own app icon. Staff enter and exit the separate admin area from the header. Passwords require at least seven characters. Account instructions supersede the legacy passphrase and personal-link workflow documented below.
