...
The live app includes a searchable, expandable member guide at `/help` and a separate Admin guide inside `/admin`. The member guide covers registration, manual payment through Connor or Havo, active status, picks, changes, deadlines, results, standings, rollovers, password help and home-screen installation. The Admin guide covers entries, payments, roles, account support, pick overrides, scores, rollovers, rounds, settings, backups and troubleshooting. A persistent Refresh button is available on desktop, tablet and phone; it waits for pending admin saves before reloading. Owner and admin access is separate from competition payment, so staff entries can be marked paid in Entries & payments like any member entry. The signed-in phone navigation is pinned as four equal-width app tabs.
