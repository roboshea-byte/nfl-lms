...
When every game in an NFL week is final, the app calculates the outcomes and begins a six-hour review window. The next week’s picks then unlock automatically on the next normal app refresh or 45-second live poll. If that week decides the competition, the same six-hour timer archives the completed round and opens the following round from the next NFL week. An owner or admin can use **Start Round N now** in Settings during a completed-round review window; this uses the same transition and does not change the automatic timing of any later week or round. A new round starts empty, so members decide whether to enter and each new entry needs payment approval.
...
The live app includes a searchable, expandable member guide at `/help` and a separate role-aware Admin guide inside `/admin`. The member guide covers registration, manual payment through Connor or Havo, active status, picks, changes, deadlines, results, standings, rollovers, password help and home-screen installation. Admins receive the complete day-to-day area for members, entries, payments, pick overrides, scores, rollovers and troubleshooting. The owner also receives role assignment, competition settings, announcements, round controls, imports, backups and reset tools. A quiet Refresh icon sits at the top right on desktop, tablet and phone; it waits for pending admin saves before reloading. The NFL LMS header logo links signed-in users to their Dashboard and signed-out visitors to the public home page. Owner and admin access is separate from competition payment, so staff entries can be marked paid in Entries & payments like any member entry. The signed-in phone navigation is pinned as four equal-width app tabs.

## Admin explainer video

The reusable Remotion project is in [`video/`](video/README.md). It renders a 119-second, 1080p narrated admin walkthrough covering registration, payments, member support, pick overrides, results, announcements, deadlines and rollovers. Run `npm install` and `npm run render` from that folder; the MP4 is written to `video/output/nfl-lms-admin-guide.mp4`.

# Accounts update (6 September 2026)

The live app now uses owner, admin and member accounts. See [account setup and operations](docs/ACCOUNTS.md). New players are directed to Register, where first name, last name, email and password are required; the login page clearly separates new registration from existing-member sign-in. A legacy single-name account cannot pick until an admin corrects it. Admins can correct first and last names without affecting the hidden account UUID, payments, picks or history. Members can add several independently managed entries from Account; the app numbers them under the member’s full name and every new entry must be marked paid by an admin before it can pick. Dashboard is the home page and includes the rules, 32-team grid and current weekly fixtures. Member navigation is Dashboard, Picks, Results and Standings; Picks also contains pick history. Standings lists each active entry on a separate line with every revealed team it has picked, alongside the prize pot, weekly status and live feed. On phones these four sections use a fixed icon tab bar at the bottom. The site can be saved to a phone home screen as NFL LMS with its own app icon. Staff enter and exit the separate admin area from the header. The owner assigns Admin in Members & roles and presses Save; the active member session remains valid and gains day-to-day admin access immediately. Manage picks lets owners/admins add, replace or clear a selection for any entry, including their own, and deliberately override the normal member deadline; each override is audited. Site settings, role assignment, the spreadsheet importer, backups, restore and reset tools are owner-only. Passwords require at least seven characters. Account instructions supersede the legacy passphrase and personal-link workflow documented below.
