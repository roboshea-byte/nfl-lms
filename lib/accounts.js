...
function createAccounts({db,now=()=>Date.now(),ownerEmail=()=>process.env.OWNER_EMAIL,ownerToken=()=>process.env.OWNER_SETUP_TOKEN,secure=process.env.NODE_ENV!=='development'}){
 ...
 async function context(req){
  ...
  const actor=safeUser(account);if(account.role==='owner'&&account.view_account_id){const target=(await query('SELECT * FROM accounts WHERE id=$1 AND disabled=false',[account.view_account_id])).rows[0];if(target&&target.role!=='owner'){const viewed=safeUser(target),effective={...viewed,role:'member'};return {user:effective,actor,viewingAs:{owner:actor,member:viewed}};}await query('UPDATE account_sessions SET view_account_id=NULL WHERE token_hash=$1',[sessionHash]);}
    ...
    if(action==='view-as'||action==='view-as-stop'){
     ...
     result=await transaction(async c=>{
      ...
      if(action==='role'||action==='disable'){
       ...
       await c.query('DELETE FROM account_sessions WHERE account_id=$1',[target.id]);await c.query('DELETE FROM account_resets WHERE account_id=$1',[target.id]);await audit(c,actor,'account.'+action,target.id,{role:b.role,disabled:b.disabled});return {};
      }
