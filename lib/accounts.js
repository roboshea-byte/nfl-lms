...
function createAccounts({db,now=()=>Date.now(),ownerEmail=()=>process.env.OWNER_EMAIL,ownerToken=()=>process.env.OWNER_SETUP_TOKEN,secure=process.env.NODE_ENV!=='development'}){
 ...
 async function user(req){const value=cookie(req);if(!value)return null;const {rows}=await query('SELECT a.* FROM accounts a JOIN account_sessions s ON a.id=s.account_id WHERE s.token_hash=$1 AND s.expires_at>$2 AND a.disabled=false',[digest(value),new Date(now())]);const account=rows[0];if(account&&account.email.toLowerCase()===configuredOwnerEmail()&&account.role!=='owner'){await query("UPDATE accounts SET role='owner' WHERE id=$1",[account.id]);account.role='owner';}return safeUser(account);}
