  ...
  try{
   ...
   if(req.method==='GET'){
    if(action==='me'){
     ...
     result={users:(await query('SELECT id,email,name,role,disabled FROM accounts ORDER BY created_at',[])).rows,entries:(await db.read()).entries,audit:(await query('SELECT actor_id,action,target_id,details,created_at FROM audit_log ORDER BY created_at DESC LIMIT 50',[])).rows};
