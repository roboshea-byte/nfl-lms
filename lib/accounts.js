...
function createAccounts({db,now=()=>Date.now(),ownerEmail=()=>process.env.OWNER_EMAIL,ownerToken=()=>process.env.OWNER_SETUP_TOKEN,secure=process.env.NODE_ENV!=='development'}){
 ...
 async function addEntry(c,u,name,label='1'){const id=crypto.randomUUID();await c.query('INSERT INTO entries(id,code,name,label,email,paid,account_id) VALUES($1,$2,$3,$4,$5,false,$6)',[id,token(),name||u.name,label,u.email,u.id]);return id;}
 async function handler(req,res){
  ...
  try{
   ...
   if(req.method==='GET'){
    if(action==='me'){
     const S=await db.read();result={user:u,entries:u?S.entries.filter(e=>e.accountId===u.id).sort((a,b)=>(Number(a.label)||Number.MAX_SAFE_INTEGER)-(Number(b.label)||Number.MAX_SAFE_INTEGER)||a.created-b.created||a.id.localeCompare(b.id)).map(e=>({id:e.id,name:e.name,label:e.label})):[]};
    }else if(action==='members'&&staff){
     result={users:(await query('SELECT id,email,name,role,disabled FROM accounts ORDER BY created_at',[])).rows,entries:u.role==='owner'?(await db.read()).entries:[],audit:u.role==='owner'?(await query('SELECT actor_id,action,target_id,details,created_at FROM audit_log ORDER BY created_at DESC LIMIT 50',[])).rows:[]};
    ...
    if(action==='signup'){
     ...
     const lastName=typeof b.lastName==='string'?b.lastName.trim().replace(/\s+/g,' '):'';
     if(!firstName||!lastName||firstName.length>50||lastName.length>50)fail('Enter your first and last name.');
     ...
     result=await transaction(async c=>{
      ...
      const account={id:crypto.randomUUID(),email,name:`${firstName} ${lastName}`,role:reserved?'owner':'member'};
      ...
      await addEntry(c,account);await session(c,res,account.id);await audit(c,account,'account.signup',account.id);return {user:safeUser(account)};
     });
    }else{
    ...
    if(action==='password'){
     ...
     result=await transaction(async c=>{const owned=(await c.query('SELECT id,label FROM entries WHERE account_id=$1 ORDER BY created_at,id',[u.id])).rows;if(owned.length>=100)fail('Contact the organiser to add more entries.');for(let i=0;i<owned.length;i++)if(!owned[i].label)await c.query('UPDATE entries SET label=$1 WHERE id=$2',[String(i+1),owned[i].id]);const id=await addEntry(c,u,typeof b.name==='string'?b.name.trim().slice(0,100):u.name,String(owned.length+1));await audit(c,u,'entry.create',id,{entryNumber:owned.length+1});return {entryId:id};});
    }else{
     ...
     result=await transaction(async c=>{
      ...
      if(action==='assign'){
       ...
       await c.query('UPDATE entries SET account_id=$1 WHERE id=$2',[target.id,entry.id]);await audit(c,u,'entry.assign',entry.id,{accountId:target.id});return {};
      }
