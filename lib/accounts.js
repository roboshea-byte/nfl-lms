...
function createAccounts({db,now=()=>Date.now(),ownerEmail=()=>process.env.OWNER_EMAIL,ownerToken=()=>process.env.OWNER_SETUP_TOKEN,secure=process.env.NODE_ENV!=='development'}){
 ...
 const configuredOwnerEmail=()=>String(ownerEmail()||'').trim().toLowerCase();
 ...
 async function context(req){
  const value=cookie(req);if(!value)return {user:null,actor:null,viewingAs:null};const sessionHash=digest(value);
  const {rows}=await query('SELECT a.*,s.view_account_id FROM accounts a JOIN account_sessions s ON a.id=s.account_id WHERE s.token_hash=$1 AND s.expires_at>$2 AND a.disabled=false',[sessionHash,new Date(now())]);const account=rows[0];
  if(!account)return {user:null,actor:null,viewingAs:null};if(account.email.toLowerCase()===configuredOwnerEmail()&&account.role!=='owner'){await query("UPDATE accounts SET role='owner' WHERE id=$1",[account.id]);account.role='owner';}
  const actor=safeUser(account);if(account.role==='owner'&&account.view_account_id){const target=(await query('SELECT * FROM accounts WHERE id=$1 AND disabled=false',[account.view_account_id])).rows[0];if(target&&target.role!=='owner'){const viewed=safeUser(target);return {user:viewed,actor,viewingAs:{owner:actor,member:viewed}};}await query('UPDATE account_sessions SET view_account_id=NULL WHERE token_hash=$1',[sessionHash]);}
  return {user:actor,actor,viewingAs:null};
 }
 async function user(req){return (await context(req)).user;}
 ...
 async function handler(req,res){
  ...
  try{
   ...
   const auth=await context(req),u=auth.user,actor=auth.actor,staff=u&&['owner','admin'].includes(u.role);
   let result={};
   if(req.method==='GET'){
    if(action==='me'){
    ...
    if(action==='signup'){
     const reserved=email===configuredOwnerEmail();
     ...
     const signedIn=await transaction(async c=>{const current=(await c.query('SELECT * FROM accounts WHERE id=$1',[account.id])).rows[0];if(current.disabled||current.password_hash!==account.password_hash)fail('Please sign in again.',401);if(current.email.toLowerCase()===configuredOwnerEmail()&&current.role!=='owner'){await c.query("UPDATE accounts SET role='owner' WHERE id=$1",[current.id]);current.role='owner';}await session(c,res,account.id);return current;});result={user:safeUser(signedIn)};
    }
    ...
    if(action==='view-as'||action==='view-as-stop'){
     if(actor?.role!=='owner')fail('Only the owner can open another member’s account.',403);
     result=await transaction(async c=>{const sessionHash=digest(cookie(req));if(action==='view-as-stop'){await c.query('UPDATE account_sessions SET view_account_id=NULL WHERE token_hash=$1 AND account_id=$2',[sessionHash,actor.id]);await audit(c,actor,'account.view_as_stop',auth.viewingAs?.member?.id||null);return {};}
      const target=(await c.query('SELECT id,email,name,role,disabled FROM accounts WHERE id=$1',[b.userId||''])).rows[0];if(!target||target.disabled||target.role==='owner')fail('Choose an active member account.');await c.query('UPDATE account_sessions SET view_account_id=$1 WHERE token_hash=$2 AND account_id=$3',[target.id,sessionHash,actor.id]);await audit(c,actor,'account.view_as_start',target.id);return {viewingAs:safeUser(target)};});
    }else if(action==='password'){
     if(auth.viewingAs)fail('Exit member view before changing this password.',403);
     ...
     result=await transaction(async c=>{
      ...
      if(action==='entry'){
       ...
       const id=await addEntry(c,target,target.name,String(owned.length+1),round);await c.query('DELETE FROM account_round_status WHERE account_id=$1 AND round_n=$2',[target.id,round]);await audit(c,actor,'entry.create',id,{accountId:target.id,round,entryNumber:owned.length+1});return {entryId:id};
      }
      if(action==='remove-entry'){
       ...
       await c.query('DELETE FROM entries WHERE id=$1',[entry.id]);await audit(c,actor,'entry.remove',entry.id,{accountId:target.id,round});return {};
      }
      if(b.status==='not_participating'){
       ...
       await c.query('DELETE FROM entries WHERE account_id=$1 AND round_n=$2 AND paid=false',[target.id,round]);await c.query(`INSERT INTO account_round_status(account_id,round_n,status,updated_at) VALUES($1,$2,'not_participating',clock_timestamp()) ON CONFLICT(account_id,round_n) DO UPDATE SET status='not_participating',updated_at=clock_timestamp()`,[target.id,round]);await audit(c,actor,'round.not_participating',target.id,{round});return {};
      }
      if(b.status==='not_entered'){await c.query('DELETE FROM account_round_status WHERE account_id=$1 AND round_n=$2',[target.id,round]);await audit(c,actor,'round.clear_status',target.id,{round});return {};}
     ...
     result=await transaction(async c=>{
      ...
      if(action==='assign'){
       ...
       await c.query('UPDATE entries SET account_id=$1,name=$2,email=$3 WHERE id=$4',[target.id,target.name,target.email,entry.id]);await audit(c,actor,'entry.assign',entry.id,{accountId:target.id});return {};
      }
      ...
      if(action==='name'){
       ...
       const name=personName(b);await c.query('UPDATE accounts SET name=$1 WHERE id=$2',[name,target.id]);await c.query('UPDATE entries SET name=$1 WHERE account_id=$2',[name,target.id]);await audit(c,actor,'account.name',target.id,{name});return {name};
      }
      if(action==='role'||action==='disable'){
       if(current.role!=='owner'||target.role==='owner'||target.email.toLowerCase()===configuredOwnerEmail())fail('Only the owner can manage other users’ roles and account access.',403);
       if(action==='role'){if(!['admin','member'].includes(b.role))fail('Choose member or admin.');await c.query('UPDATE accounts SET role=$1 WHERE id=$2',[b.role,target.id]);await c.query('DELETE FROM account_resets WHERE account_id=$1',[target.id]);}
       else {if(typeof b.disabled!=='boolean')fail('Invalid account status.');await c.query('UPDATE accounts SET disabled=$1 WHERE id=$2',[b.disabled,target.id]);await c.query('DELETE FROM account_sessions WHERE account_id=$1',[target.id]);await c.query('DELETE FROM account_resets WHERE account_id=$1',[target.id]);}
       await audit(c,actor,'account.'+action,target.id,{role:b.role,disabled:b.disabled});return {};
      }
      if(action==='reset-link'){
       if(target.role==='owner'||(current.role!=='owner'&&target.role!=='member'))fail('You cannot reset this account.',403);
       const value=token();await c.query('DELETE FROM account_resets WHERE account_id=$1',[target.id]);await c.query('INSERT INTO account_resets(token_hash,account_id,expires_at) VALUES($1,$2,$3)',[digest(value),target.id,new Date(now()+3600000)]);await audit(c,actor,'account.reset_link',target.id);return {resetToken:value};
      }
 ...
 return {user,context,checkRequest,handler};
}
