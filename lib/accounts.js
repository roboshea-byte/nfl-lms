...
function createAccounts({db,now=()=>Date.now(),ownerEmail=()=>process.env.OWNER_EMAIL,ownerToken=()=>process.env.OWNER_SETUP_TOKEN,secure=process.env.NODE_ENV!=='development'}){
 ...
 async function addEntry(c,u,name){const id=crypto.randomUUID();await c.query('INSERT INTO entries(id,code,name,email,paid,account_id) VALUES($1,$2,$3,$4,false,$5)',[id,token(),name||u.name,u.email,u.id]);return id;}
 async function handler(req,res){
  ...
  try{
   ...
   if(req.method==='GET'){
    if(action==='me'){
     const S=await db.read();result={user:u,entries:u?S.entries.filter(e=>e.accountId===u.id).map(e=>({id:e.id,name:e.name,label:e.label})):[]};
    ...
    if(action==='password'){
     ...
     result=await transaction(async c=>{const count=(await c.query('SELECT id FROM entries WHERE account_id=$1',[u.id])).rows.length;if(count>=100)fail('Contact the organiser to add more entries.');const id=await addEntry(c,u,typeof b.name==='string'?b.name.trim().slice(0,100):u.name);await audit(c,u,'entry.create',id);return {entryId:id};});
    }else{
