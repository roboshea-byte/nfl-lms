...
const HEAD_COACH_FALLBACK={ARI:'Mike LaFleur'};
...
async function fetchJSON(fetchImpl,url){
  const response=await fetchImpl(url,{headers:{Accept:'application/json','Accept-Language':'en-GB,en;q=0.9','User-Agent':'Mozilla/5.0 (compatible; NFL-LMS/1.0; +https://nfl-lms.vercel.app)'},signal:typeof AbortSignal!=='undefined'&&AbortSignal.timeout?AbortSignal.timeout(8000):undefined});
...
function createTeamInfoHandler({fetchImpl=(...args)=>fetch(...args),now=()=>Date.now()}={}){
  return async function teamInfo(req,res){
    ...
    try{
      ...
      const base=`https://site.web.api.espn.com/apis/site/v2/sports/football/nfl/teams/${abbr.toLowerCase()}`;
