...
function createHandlers({db,auth,now=()=>Date.now(),adminKey=()=>process.env.ADMIN_KEY,fetchImpl=(...args)=>fetch(...args),newCode=()=>crypto.randomBytes(8).toString('base64url').slice(0,10)}) {
  ...
  async function stateWithAutomaticRound(){
    let S=await db.read(),r=L.currentRound(S),comp=L.computeRound(S,r),advanceAt=L.roundAdvanceAt(r),time=now();
    if(comp.complete&&(!r.completedAt||(advanceAt!==null&&time>=advanceAt)))S=await db.mutate(state=>{L.advanceRound(state,time);return structuredClone(state);});
    return S;
  }
