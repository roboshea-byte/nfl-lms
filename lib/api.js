...
function createHandlers({db,auth,now=()=>Date.now(),adminKey=()=>process.env.ADMIN_KEY,fetchImpl=(...args)=>fetch(...args),newCode=()=>crypto.randomBytes(8).toString('base64url').slice(0,10)}) {
  ...
  function endpoint(name,method,admin,fn) {
    handlers[name]=async(req,res)=>{
      ...
      try {
        ...
        req.user=auth?await auth.user(req):null;
  ...
  endpoint('me','GET',false,async(req)=>{
    const S=await db.read();const e=S.entries.find(e=>auth?e.id===req.query?.entryId && e.accountId===req.user?.id:e.code===req.query?.code);
  ...
  endpoint('pick','POST',false,async(req)=>{
    ...
    },auth?{actorId:req.user?.id,action:'competition.'+req.url?.split('?')[0]}:undefined);
  });
  ...
  endpoint('adminState','POST',true,async(req)=>{
    ...
    return db.mutate(S=>{
      ...
      S.entries=b.entries.map(e=>{
        ...
        return {accountId:old?.accountId||null,id:e.id,code,name:e.name.trim(),label:e.label||'',email:e.email||'',paid:e.paid,rolloverPayments:structuredClone(e.rolloverPayments??old?.rolloverPayments??{}),created:old?.created||(Number.isFinite(e.created)?e.created:now())};
      });
