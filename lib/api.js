...
const publicEntry = ({id,name,label,paid,rolloverPayments}) => ({id,name,label,paid,rolloverPayments:rolloverPayments||{}});
function statePayload(S,admin,now) {
  ...
  const state={settings:{...S.settings,wipeoutResetTeams:true},entries:admin?S.entries:S.entries.map(publicEntry),picks,results:S.results,rounds:S.rounds,admin};
  ...
  endpoint('adminState','POST',true,async(req)=>{
    ...
    return db.mutate(S=>{
      ...
      S.entries=b.entries.map(e=>{
        ...
        return {id:e.id,code,name:e.name.trim(),label:e.label||'',email:e.email||'',paid:e.paid,rolloverPayments:structuredClone(e.rolloverPayments??old?.rolloverPayments??{}),created:old?.created||(Number.isFinite(e.created)?e.created:now())};
      });
      S.settings={fee:b.settings.fee,tieRule:b.settings.tieRule,wipeoutResetTeams:true,missedPick:b.settings.missedPick,title:text(b.settings.title,200)?b.settings.title:'Last Man Standing'};
