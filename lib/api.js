...
const publicEntry = ({id,name,label,paid}) => ({id,name,label,paid});
function statePayload(S,admin,now) {
  const picks={};
  for (const [id,weeks] of Object.entries(S.picks)) {
    picks[id]={};
    for (const [week,team] of Object.entries(weeks)) picks[id][week]=admin || (L.kickoff(team,+week)!==null && L.kickoff(team,+week)<=now) ? team : 'HIDDEN';
  }
  const state={settings:S.settings,entries:admin?S.entries:S.entries.map(publicEntry),picks,results:S.results,rounds:S.rounds,admin};
  // A content revision changes at kick-off even when nobody writes to the database.
  return {...state,updatedAt:hash(state),...(admin?{revision:revision(S)}:{}),serverTime:new Date(now).toISOString()};
}
function mePayload(S,entry,now) {return {entry:publicEntry(entry),picks:S.picks[entry.id]||{},state:statePayload(S,false,now),serverTime:new Date(now).toISOString()};}
function bodyOf(req) {
  let b=req.body;
...
function validateAdmin(b) {
  ...
  if (!Array.isArray(b.entries) || b.entries.length>10000 || b.entries.some(e=>!e || !safeId(e.id) || !text(e.name,200) || !e.name.trim() || !text(e.label||'',200) || !text(e.email||'',320) || typeof e.paid!=='boolean') || new Set(b.entries.map(e=>e.id)).size!==b.entries.length) fail('bad_state','Each entry needs a unique ID, name and payment status.');
  ...
  for (let i=0;i<b.rounds.length;i++) {
    const r=b.rounds[i];
    if (!r || r.n!==i+1 || !Number.isInteger(r.startWeek) || r.startWeek<1 || r.startWeek>18 || r.startWeek<=end || (r.endWeek!==null && (!Number.isInteger(r.endWeek) || r.endWeek<r.startWeek || r.endWeek>18)) || (r.winnerIds!==null && (!Array.isArray(r.winnerIds) || r.winnerIds.some(id=>!safeId(id))))) fail('bad_state','Check round numbers and week boundaries.');
    end=r.endWeek||18;
  }
  ...
  for (const [id,r] of Object.entries(b.results)) {
    if (!schedule.games.some(g=>g.id===id) || !r || typeof r.final!=='boolean' || [r.hs,r.as].some(v=>v!==null && (!Number.isInteger(v) || v<0 || v>200)) || (r.final && (r.hs===null || r.as===null))) fail('bad_state','Check the game IDs and scores.');
  }
...
function createHandlers({db,now=()=>Date.now(),adminKey=()=>process.env.ADMIN_KEY,fetchImpl=(...args)=>fetch(...args),newCode=()=>crypto.randomBytes(8).toString('base64url').slice(0,10)}) {
  const handlers={};
  function endpoint(name,method,admin,fn) {
    handlers[name]=async(req,res)=>{
      ...
      try {
        ...
        const authed=isAdmin(req,adminKey());
        if (admin && !authed) fail('unauthorised','Enter the organiser passphrase.',401);
  ...
  endpoint('state','GET',false,async(req,admin)=>statePayload(await db.read(),admin,now()));
  endpoint('me','GET',false,async(req)=>{
    const S=await db.read();const e=S.entries.find(e=>e.code===req.query?.code);
    if (!e) fail('unknown_code','That personal link was not found. Ask the organiser for your link.',404);
    return mePayload(S,e,now());
  });
  endpoint('pick','POST',false,async(req)=>{
    const {code,week,team}=bodyOf(req);
    return db.mutate(S=>{
      const e=S.entries.find(e=>e.code===code);
      if (!e) fail('unknown_code','That personal link was not found.');
      if (!e.paid) fail('unpaid',"You're not marked as paid yet, contact the organiser");
      const r=L.currentRound(S),comp=L.computeRound(S,r);
      ...
      const existing=S.picks[e.id]?.[week],time=now(); // Read clock after obtaining the transaction lock.
      const kicked=t=>L.kickoff(t,week)!==null && L.kickoff(t,week)<=time;
      if ((existing && kicked(existing)) || (!existing && L.weekFirstKickoff(week)<=time) || (team!==null && kicked(team))) fail('locked','Picks are locked once the relevant game has kicked off. A first pick must be in before the week starts.');
      if (team!==null && (typeof team!=='string' || !L.teamGame(team,week))) fail('bye','That team is not playing this week.');
      if (team!==null && Object.hasOwn(L.usedTeams(S,e.id,r,comp,week),team)) fail('used','You have already used that team in this round.');
      if (team===null) {if(S.picks[e.id]) delete S.picks[e.id][week];}
      else (S.picks[e.id] ||= {})[week]=team;
      return mePayload(S,e,time);
    });
  });
  endpoint('adminPick','POST',true,async(req)=>{
    const {entryId,week,team}=bodyOf(req);
    return db.mutate(S=>{
      if (!S.entries.some(e=>e.id===entryId)) fail('unknown_entry','That entry no longer exists.');
  ...
  endpoint('adminState','POST',true,async(req)=>{
    ...
    return db.mutate(S=>{
      ...
      S.entries=b.entries.map(e=>{
        ...
        return {id:e.id,code,name:e.name.trim(),label:e.label||'',email:e.email||'',paid:e.paid,created:old?.created||(Number.isFinite(e.created)?e.created:now())};
      });
      S.settings={fee:b.settings.fee,tieRule:b.settings.tieRule,wipeoutResetTeams:b.settings.wipeoutResetTeams,missedPick:b.settings.missedPick,title:text(b.settings.title,200)?b.settings.title:'Last Man Standing'};
      S.rounds=b.rounds.map(r=>({n:r.n,startWeek:r.startWeek,endWeek:r.endWeek,winnerIds:r.winnerIds}));
  ...
  endpoint('fetchResults','POST',true,async(req)=>{
    ...
    return db.mutate(S=>{
      ...
      for(const ev of d.events){
        ...
        S.results[g.id]={hs:+home.score,as:+away.score,final:!!ev.status.type.completed};count++;if(ev.status.type.completed)final++;
      }
