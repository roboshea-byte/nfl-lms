...
const normalSettings=s=>({fee:Number.isFinite(s?.fee)?s.fee:20,tieRule:s?.tieRule||'loss',wipeoutResetTeams:true,missedPick:s?.missedPick||'eliminate',title:text(s?.title,200)?s.title:'Last Man Standing',announcement:(s?.announcement??'').trim(),announcementEnabled:s?.announcementEnabled??false,announcementType:s?.announcementType??'update'});
...
function createHandlers({db,auth,now=()=>Date.now(),adminKey=()=>process.env.ADMIN_KEY,fetchImpl=(...args)=>fetch(...args),newCode=()=>crypto.randomBytes(8).toString('base64url').slice(0,10)}) {
  ...
  endpoint('state','GET',false,async(req,admin)=>statePayload(await stateWithAutomaticRound(),admin,now(),admin||!!req.user));
  endpoint('me','GET',false,async(req)=>{
    ...
    return mePayload(S,e,now(),!!auth&&!!req.user);
  });
  endpoint('pick','POST',false,async(req)=>{
    ...
    return db.mutate(S=>{
      ...
      return mePayload(S,e,time,!!auth&&!!req.user);
  ...
  endpoint('adminState','POST',true,async(req)=>{
    const b=bodyOf(req);validateAdmin(b);const owner=!auth||req.user?.role==='owner';
    if(!owner&&b.replacePicks!==undefined)fail('owner_required','Only the owner can restore or replace competition data.',403);
    return db.mutate(S=>{
      ...
      if(!owner&&(hash(normalSettings(b.settings))!==hash(normalSettings(S.settings))||hash(b.rounds)!==hash(S.rounds)))fail('owner_required','Only the owner can change competition settings or round controls.',403);
      ...
      if(owner){
        S.settings=normalSettings(b.settings);
