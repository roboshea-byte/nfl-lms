function createLMSLogic(SCHEDULE){
  ...
  function prizePot(S,r,comp){
    ...
    return S.settings.fee*S.entries.reduce((n,e)=>n+Number(!!e.paid)+starts.filter(w=>paidForReset(e,r,w)).length,0);
  }
  ...
  function computeRound(S, r){
    ...
    let alive = new Set(S.entries.filter(e=>e.paid).map(e=>e.id));
    const out = {weeks:{}, events:[], winnerIds:null, complete:false, resetWeek:r.startWeek, finalWeek:null};
    for(let w=r.startWeek; w<=roundEndWeek(r); w++){
      ...
      if(settled && alive.size>0 && losers.length===alive.size){
        ...
        alive = new Set(S.entries.filter(e=>paidForReset(e,r,w+1)).map(e=>e.id));
  ...
  function aliveAtWeek(S, comp, w){
    ...
    return new Set(S.entries.filter(e=>e.paid).map(e=>e.id));
  }
  ...
  return {weekDeadline, WEEKS, GAMES, gamesByWeek, teamGame, teamResult, weekSettled, weekHasResults, currentRound, roundEndWeek, defaultWeek, isPickLost, computeRound, usedTeams, aliveAtWeek, isAliveForWeek, kickoff, weekFirstKickoff, paymentWeek, isPaid, prizePot};
}
