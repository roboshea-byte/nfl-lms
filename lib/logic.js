function createLMSLogic(SCHEDULE){
  ...
  function computeRound(S, r){
    ...
    for(let w=r.startWeek; w<=roundEndWeek(r); w++){
      ...
      if(settled && alive.size>0 && losers.length===alive.size){
        wipeout=true;
        alive = new Set(all);
        if(S.settings.wipeoutResetTeams) out.resetWeek = w+1;
      ...
      if(settled && alive.size===1 && all.length>1){ out.winnerIds=[...alive]; out.complete=true; out.finalWeek=w; out.events.push({week:w,type:'winner',ids:[...alive]}); break; }
  ...
  return {WEEKS, GAMES, gamesByWeek, teamGame, teamResult, weekSettled, weekHasResults, currentRound, roundEndWeek, defaultWeek, isPickLost, computeRound, usedTeams, aliveAtWeek, isAliveForWeek, kickoff, weekFirstKickoff};
}
